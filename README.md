...
3. requests + parallel : 80,450ms (1m 20s 450ms)
	- 114.14s user 21.95s system 169% cpu
4. requests + parallel + 전처리 optimization : 76,010ms (1m 16s 10ms)
	- 110.25s user 19.14s system 170% cpu



# E. 시행착오

## a. thread pool 갯수 조절로 인한 성능 향상 실패

### 1. 의문증
Q. 쓰레드 여러개를 병렬로 동시에 1000개 페이지를 크롤링 할 때,
쓰레드 갯수를 최적갯수로 지정해주면, latency가 더 좋아지지 않을까?
...
- before 최적화 시도
	- Execution time: 75.63232612609863 seconds
- after 최적화 시도
	1. 8 threads(8 core)
		- Execution time: 79.71867609024048 seconds
	2. 9 threads(8 core + 1)
		- Execution time: 77.72524785995483 seconds
	3. 10 threads(8 core + 2)
		- Execution time: 77.08852696418762 seconds
	4. 11 threads(8 core + 3)
		- Execution time: 78.82938313484192 seconds
	5. 12 threads (8 core + 4)
		- Execution time: 78.09761595726013 seconds
	6. 13 threads (8 core + 5)
		- Execution time: 77.02442026138306 seconds
	7. 14 threads (8 core + 6)
		- Execution time: 87.12706995010376 seconds
	8. 15 threads(8 core + 7)
		- Execution time: 94.73707699775696 seconds
	9. 16 threads(8 core + 8)
		- Execution time: 96.22088408470154 seconds

### 4. 느낀점

core 갯수(8) + 2가 가장 성능이 좋았으나(77.08s),\
...
부검 결과, 이 코드에서 필터링 하고자 하는 특수문자들은 바로 다음 코드인 `re.findall(r'\b[가-힣]{2,15}\b', refinedData)` , 한글필터링 코드에서 걸러지기 때문에 삭제한다.
...
- ex2. 무엇보다도, 금지어 제거 부분에서 금지어 리스트를 File I/O로 가져온다는 점에서 매우 느렸다.
